...
router.post('/respond', async (req, res) => {
  try {
    ...
    // Find the interview session first
    ...
    if (!interview) {
      return res.status(404).json({ 
        error: 'Interview session not found or not in progress' 
      });
    }
    ...
    const updatedInterview = await InterviewSession.findByIdAndUpdate(interviewId, {
